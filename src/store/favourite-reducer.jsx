...
export const favouriteReducer = (state = initialState, action) => {
    switch (action.type) {
        case "ADD_FAVOURITE_RECIPE":
            return { ...state, favouriteRecipes: state.favouriteRecipes.push(action.recipe)};
        case "DELETE_FAVOURITE_RECIPE":
            return { ...state, favouriteRecipes: state.favouriteRecipes.filter(recipe => recipe.id !== action.recipeId) };
        default:
            return state;
    }
};
